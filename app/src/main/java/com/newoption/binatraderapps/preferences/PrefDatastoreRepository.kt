package com.newoption.binatraderapps.preferences
...
class PrefDatastoreRepository(val context: Context) {
    ...
    companion object {
        ...
        private val INSTALL_REFERRER_KEY = stringPreferencesKey(AppConstants.INSTALL_REFERRER_KEY)
    }
    ...
    suspend fun saveInstallReferrer(value: String) {
        context.dataStore.edit {
            it[INSTALL_REFERRER_KEY] = value
        }
    }

    val installReferrer: Flow<String> = context.dataStore.data
        .map {
            it[INSTALL_REFERRER_KEY] ?: ""
        }
